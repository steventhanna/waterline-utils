...
var runCreateStrategy = require('./private/run-create-strategy');
...
module.exports = function runAutoMigrations(strategy, orm, cb) {
  ...
  if (!_.isString(strategy)) {
    return cb(new Error('Strategy must be one of: `alter`, `drop`, `create` or `safe`.'));
  }
  ...
  switch(strategy){
    ...
    case 'create': runCreateStrategy(orm, cb); break;
    default: return cb(new Error('Strategy must be one of: `alter`, `drop`, `create` or `safe`.'));
  }
};
